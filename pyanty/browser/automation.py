...
from ..runtime import collect_garbage

LOCAL_DOLPHIN_API_BASE_URL = "http://localhost:3001"
DOLPHIN_DOCS_URL = (
    "https://help.dolphin-anty.com/en/articles/7127390-basic-automation-dolphin-anty"
)
JSONValue = dict[str, object] | list[object] | str | int | float | bool | None


def run_profile(profile_id: int | str, headless: bool = False) -> JSONValue:
    is_headless_str = "" if not headless else "&headless=true"
    r = requests.get(
        f"{LOCAL_DOLPHIN_API_BASE_URL}/v1.0/browser_profiles/{profile_id}/start?automation=1"
        + is_headless_str
    )
    data = r.json()
    if "error" in data:
...
def close_profile(profile_id: int | str) -> JSONValue:
    data = requests.get(
        f"{LOCAL_DOLPHIN_API_BASE_URL}/v1.0/browser_profiles/{profile_id}/stop"
    ).json()
    collect_garbage(profile_id=profile_id)
    return data


def download_driver_to_memory(driver_url: str) -> io.BytesIO:
    response = requests.get(driver_url, stream=True)

    if response.status_code != 200 or "zip" not in response.headers.get(
        "Content-Type", ""
    ):
    ...
    for data in response.iter_content(block_size):
        ...
        progress_bar = (
            f"Downloading the driver... [{progress_hash_count * '#'}"
            f"{(progress_bar_size - progress_hash_count) * '-'}] "
...
def get_dolphin_driver() -> io.BytesIO:
...
def unzip_driver_from_memory(driver_content: io.BytesIO) -> None:
    with zipfile.ZipFile(driver_content, "r") as zip_ref:
        zip_ref.extractall(path=os.getcwd())


def clean_trash(zip_files: list[str]) -> None:
...
def unpack_subarchives(system: str, architecture: str) -> None:
    ...
    for zip_file in zip_files:
        if (
            system == "Windows"
            and "win" in zip_file.lower()
            and file_architecture in zip_file
        ):
...
def select_driver_executable(system: str, architecture: str) -> str:
    unpack_subarchives(system, architecture)
    if system == "Windows":
        executable_name = (
            "chromedriver.exe" if "64" in architecture else "chromedriver_x86.exe"
        )
...
def get_driver(port: int = 9222) -> object:
...
async def get_browser(ws_endpoint: str, port: int, core: str = "pyppeteer") -> object:
    if core == "pyppeteer":
        try:
            from pyppeteer import connect
        except ImportError:
            raise ImportError(
                "To work with pyppeteer core, you need to install it with: "
        ...
        if core == "playwright":
            try:
                ...
                raise ImportError(
                    "To work with playwright core, you need to install it with: "
        ...
        browser = await p.chromium.connect_over_cdp(
            f"ws://127.0.0.1:{port}{ws_endpoint}"
        )
...
if sys.platform == "win32":
    ...
    def authorize_in_window(login: str, password: str) -> None:
        ...
        if main_window.exists():
            ...
            for desc in descendants:
                if (
                    "Email" in desc.window_text()
                    or "email" in desc.legacy_properties().get("Value", "").lower()
                ):
                    email_field = desc
                elif (
                    "Password" in desc.window_text()
                    or "password" in desc.legacy_properties().get("Value", "").lower()
                ):
                    password_field = desc
            ...
            if email_field and password_field:
                ...
                sign_in_button = main_window.descendants(
